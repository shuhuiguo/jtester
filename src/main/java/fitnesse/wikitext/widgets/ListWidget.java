...
public class ListWidget extends ParentWidget {
	...
	private String buildList(String text) throws Exception {
		...
		if (match.find()) {
			...
			if (level > this.level) {
				...
				String childItems;
				if (nextMatch.find()) {
					// If there is a item on same layer, get text until this
					// position -> child items.
					childItems = text.substring(match.end()).substring(0, nextMatch.start())
							.replaceAll("(^[\\r\\n]+)|([ \\r\\n]+$)", "");
				} else {
					// If there is no item on same layer, the text to the end
					// are child items.
	...
	protected void appendTabs(StringBuffer html) {
