...
public class CollapsableWidget extends ParentWidget {
	...
	// hack. We only set the following flag to false in the 2-arg. constructor
	// shown below. This constructor
	// is only used in WidgetBuilder and only when "!* .... *!" wiki text is
	// encountered. For those collapsable sections,
	...
	// invisible: Add to fitnesse_base.css: .invisible { line-height: 0px;
	// visibility:hidden; }
	...
	public CollapsableWidget(ParentWidget parent, ParentWidget includeParent, String title, String body,
			String cssClass, boolean collapsed) throws Exception {
		this(parent);
	...
	private String makeImportedEditLinks(PageData pageData, String title) {
		return "(edit: [[locally][" + makeLocalEditLinks(title) + "]]" + " | [[remotely]["
				+ makeRemoteLink(pageData, title) + "]])";
	}
