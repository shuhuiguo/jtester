...
public class ScanString {
	...
	public boolean matches(String match, int startsAt) {
		if (match.length() == 0)
			return false;
		if (offset + startsAt + match.length() > input.length())
			return false;
	...
	public boolean startsLine(int startAt, String startDelimiters) {
		return offset + startAt == 0 || offset + startAt == markStartOffset
				|| startDelimiters.indexOf(input.charAt(offset + startAt - 1)) >= 0;
	}
	...
	public char charAt(int startAt) {
		if (offset + startAt >= input.length())
			return 0;
