...
public class Table extends SymbolType implements Rule, Translation {
	...
	private Symbol parseCell(Parser parser, String content) {
		return (content.indexOf("!") >= 0) ? parser.parseToWithSymbols(SymbolType.EndCell,
				SymbolProvider.literalTableProvider, 1) : parser.parseTo(SymbolType.EndCell, 1);
	}

	private boolean containsNewLine(Symbol cell) {
		for (Symbol child : cell.getChildren()) {
			if (child.isType(SymbolType.Newline))
				return true;
		}
		return false;
	}
