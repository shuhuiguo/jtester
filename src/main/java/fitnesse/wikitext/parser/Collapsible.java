...
public class Collapsible extends SymbolType implements Rule, Translation {
	...
	public Maybe<Symbol> parse(Symbol current, Parser parser) {
		...
		return new Maybe<Symbol>(current.putProperty(State, state).add(titleText).add(bodyText));
	}
	...
	public String toTarget(Translator translator, Symbol symbol) {
		...
		return option.equals(Collapsible.Invisible) ? makeInvisibleSection(body) : generateHtml(option, title, body);
	}
	...
	private static String bodyClass(String state) {
		return state.equals(Collapsible.Open) ? "collapsable" : state.equals(Collapsible.Closed) ? "hidden"
				: "invisible";
	}
}
