...
public class FitNesseSuite extends ParentRunner<String> {
	...
	/**
	 * The <code>SuiteFilter</code> annotation specifies the suite filter of the
	 * Fitnesse suite to be run, e.g.: fasttests
	 */
	@Retention(RetentionPolicy.RUNTIME)
	@Target(ElementType.TYPE)
	...
	/**
	 * The <code>FitnesseDir</code> annotation specifies the absolute or
	 * relative path to the directory in which FitNesseRoot can be found
	 */
	@Retention(RetentionPolicy.RUNTIME)
	@Target(ElementType.TYPE)
	...
	private JUnitHelper createJUnitHelper(final RunNotifier notifier) {
		JUnitHelper jUnitHelper = new JUnitHelper(this.fitNesseDir, this.outputDir,
				new JUnitRunNotifierResultsListener(notifier, suiteClass));
