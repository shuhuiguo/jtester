...
public class WikiPageAction {
	...
	public boolean equals(Object o) {
		...
		if (newWindow != that.newWindow)
			return false;

		return eq(linkName, that.linkName) && eq(pageName, that.pageName) && eq(query, that.query)
				&& eq(shortcutKey, that.shortcutKey);
	}

	private boolean eq(String thisString, String thatString) {
		return (thisString != null && thatString != null && thisString.equals(thatString))
