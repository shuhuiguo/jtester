...
public class PageCrawlerImpl implements PageCrawler {
	...
	public void traverse(WikiPage context, TraversalListener listener) throws Exception {
		if (context.getClass() == SymbolicPage.class)
			return;
		// TODO MdM Catch any exception thrown by the following and add the page
		// name to the Exception message.
	...
	/*
	 * Todo: RcM. All calls to getPage should actually come here, and be
	 * relative to the current page, not the parent page. It was a gross error
	 * to have the whole wiki know that references were relative to the parent
	 * instead of the page.
	 */
	public WikiPage getSiblingPage(WikiPage page, WikiPagePath pathRelativeToSibling) throws Exception {
		...
		if (pathRelativeToSibling.isSubPagePath()) {
			...
			if (ancestor != null)
				return getPage(ancestor, pathRelativeToSibling.getRest());
