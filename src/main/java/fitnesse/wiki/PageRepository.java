	...
	public List<WikiPage> findChildren(ExternalSuitePage parent) throws Exception {
		...
		for (String child : fileSystem.list(parent.getFileSystemPath())) {
			...
			if (child.endsWith(".html")) {
				children.add(new ExternalTestPage(childPath, WikiWordWidget.makeWikiWord(child.replace(".html", "")),
						parent, fileSystem));
			} else if (hasHtmlChild(childPath)) {
				children.add(new ExternalSuitePage(childPath, WikiWordWidget.makeWikiWord(child), parent, fileSystem));
			}
		}
		return children;
	}
