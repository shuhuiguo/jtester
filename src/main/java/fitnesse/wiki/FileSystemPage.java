...
public class FileSystemPage extends CachingPage {
	...
	protected synchronized void saveAttributes(final WikiPageProperties attributes) throws Exception {
		...
		try {
			...
			System.err
					.println("Failed to save properties file: \"" + propertiesFilePath + "\" (exception: " + e + ").");
			e.printStackTrace();
			throw e;
		} finally {
			if (output != null) {
	...
	protected WikiPage createChildPage(final String name) throws Exception {
		// return new FileSystemPage(getFileSystemPath(), name, this,
		// this.versionsController);
		...
		private void invokeCmMethod(String method, String newPagePath) throws Exception {
			if (getCmSystemClassName() != null) {
				try {
					...
					System.err.println("Could not invoke static " + method + "(path,payload) of "
							+ getCmSystemClassName());
					e.printStackTrace();
				}
			}
		}
