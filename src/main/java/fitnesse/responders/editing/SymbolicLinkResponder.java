...
public class SymbolicLinkResponder implements Responder {
	...
	private void renameSymbolicLink(Request request, WikiPage page) throws Exception {
		String linkToRename = (String) request.getInput("rename"), newName = (String) request.getInput("newname");

		if (page.hasChildPage(newName)) {
			response = new ErrorResponder(resource + " already has a child named " + newName + ".").makeResponse(
					context, null);
	...
	private void addSymbolicLink(Request request, WikiPage page) throws Exception {
		...
		if (isFilePath(linkPath) && !isValidDirectoryPath(linkPath)) {
			String message = "Cannot create link to the file system path, <b>" + linkPath + "</b>."
					+ "<br/> The canonical file system path used was <b>"
					+ createFileFromPath(linkPath).getCanonicalPath() + ".</b>"
			...
			response = new ErrorResponder("The page to which you are attempting to link, " + Utils.escapeHTML(linkPath)
					+ ", doesn't exist.").makeResponse(context, null);
			response.setStatus(404);
		} else if (page.hasChildPage(linkName)) {
			response = new ErrorResponder(resource + " already has a child named " + linkName + ".").makeResponse(
					context, null);
			response.setStatus(412);
		} else {
			PageData data = page.getData();
	...
	private boolean isInternalPageThatDoesntExist(String linkPath) throws Exception {
		...
		WikiPage start = path.isRelativePath() ? page.getParent() : page; // TODO
																			// -AcD-
																			// a
																			// better
																			// way?
