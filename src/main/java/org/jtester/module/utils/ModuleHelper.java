...
import org.jtester.module.ModulesLoader;
import org.jtester.module.ModulesRepository;
import org.jtester.module.TestListener;
import org.jtester.module.core.Module;

/**
 * Core class of the Unitils library, and the main entry point that gives access
 ...
 * {@link #init(Properties)} method. Normally, only one instance of Unitils
 ...
 * {@link ModulesRepository} is used to initialize and maintain the modules.
 * <p/>
 * Unitils itself is also implemented as a module. In fact, an instance of
 * Unitils behaves like a module who's behaviour is defined by the added
...
public class ModuleHelper {
	 ...
	 */
	public static synchronized ModuleHelper getInstance() {
		if (instance == null) {
	 ...
	 *            the singleton instance
	 */
	public static void setInstance(ModuleHelper instance) {
		ModuleHelper.instance = instance;
	}
	...
	/**
	 * Initializes Unitils with the given configuration. All the modules that
	 * are configured in the given configuration are also created and
	...
	public void init(Properties configuration) {
		...
		testListener = new JTesterTestListener();
		afterInitModules();
	}

	/**
	 * Gives all modules the opportunity to performs initialization that can
	 * only work after all other modules have been initialized
	 */
	...
	/**
	 * Verifies that we're not working with a distribution that includes the
	 * necessary classes from spring, while spring is in the classpath anyway.
	 * 
	 * @param configuration
	 *            The configuration
	 */
	protected void verifyPackaging(Properties configuration) {
		...
		if (isClassAvailable(springCoreClassName) && isClassAvailable(unitilsPackagedWithSpring)) {
			throw new IllegalStateException(
					"It appears that you're using the unitils distribution that is packaged with "
	...
	/**
	 * Returns the {@link ModulesRepository} that provides access to the modules
	 * that are configured in unitils.
	 * 
	...
	/**
	 * Returns the {@link TestContext} that, during the execution of the test
	 * suite, keeps track of the current test object, class and test method that
	 * are executing.
	 * 
	...
	/**
	 * Returns all properties that are used to configure unitils and the
	 * different modules.
	 * 
	...
	/**
	 * Configures all unitils modules using the given <code>Properties</code>
	 * object, and stores them in a {@link ModulesRepository}. The configuration
	 * of the modules is delegated to a {@link ModulesLoader} instance.
	 * 
	 * @param configuration
	 *            The config, not null
	 ...
	 * {@link TestContext} returned by {@link ModuleHelper#getTestContext()} is
	 * correctly set to the current test class, test object and test method.
	 */
	private class JTesterTestListener extends TestListener {
