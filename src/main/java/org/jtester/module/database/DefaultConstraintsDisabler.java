 ...
 * foreign key, check and not-null constraints on the configured database
 * schemas. Primary key constraints will not be disabled.
 * 
 * @author Tim Ducheyne
 * @author Filip Neven
...
public class DefaultConstraintsDisabler extends BaseDatabaseAccessor implements ConstraintsDisabler {
	...
	private final static Logger log4j = Logger.getLogger(DefaultConstraintsDisabler.class);
	...
	public void disableConstraints() {
		for (DbSupport dbSupport : dbSupports) {
			log4j.info("Disabling constraints in database schema " + dbSupport.getSchemaName());
	...
	/**
	 * Disables all referential constraints (e.g. foreign keys) on all tables in
	 * the schema
	 * 
	 * @param dbSupport
	...
	protected void disableReferentialConstraints(DbSupport dbSupport) {
		try {
			...
			log4j.error("Unable to remove referential constraints.", t);
		}
	}
	...
	protected void disableValueConstraints(DbSupport dbSupport) {
		try {
			...
			log4j.error("Unable to remove value constraints.", t);
		}
	}
}