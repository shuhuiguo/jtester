...
public class AccessibleXStream {
	 ...
	 */
	public AccessibleXStream(ReflectionProvider reflectionProvider, HierarchicalStreamDriver driver,
			ClassLoader classLoader, Mapper mapper, ConverterLookup converterLookup, ConverterRegistry converterRegistry) {
		jvm = new JVM();
	...
	private Mapper buildMapper() {
		...
		if (JVM.is15()) {
			mapper = buildMapperDynamically(ANNOTATION_MAPPER_TYPE, new Class[] { Mapper.class,
					ConverterRegistry.class, ClassLoader.class, ReflectionProvider.class, JVM.class }, new Object[] {
	 ...
	 *             if mapper is not really a deprecated {@link ClassMapper}
	 *             instance
	...
	/**
	 * Creates an ObjectInputStream that deserializes a stream of objects from a
	 * reader using XStream. <h3>Example</h3>
	 * 
	 * <pre>
