...
public class RemoteInvokerRegister {
	private final static Logger log4j = Logger.getLogger(RemoteInvokerRegister.class);
	...
	public static void registerSpringBeanRemoteOnClient(final DefaultListableBeanFactory beanFactory,
		...
		for (Field field : fields_HttpInvoker) {
			...
			if (beanFactory.containsBeanDefinition(beanID)) {
				log4j.info(String.format("spring bean[%s] has been defined in application context!", beanID));
				return;
			}
		...
		while (en.hasMoreElements()) {
			...
			for (String type : types) {
				...
				if (type.equalsIgnoreCase("httpinvoker")) {
					...
					log4j.info("register httpinvoker bean[/httpinvoker/" + bean + "] successfully.");
					...
					log4j.info("register httpinvoker bean[/hessian/" + bean + "] successfully.");
				} else {
