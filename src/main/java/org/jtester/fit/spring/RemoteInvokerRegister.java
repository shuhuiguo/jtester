...
public class RemoteInvokerRegister {
	private static Logger log = Logger.getLogger(RemoteInvokerRegister.class);
	...
	public static void registerSpringBeanRemoteOnClient(final DefaultListableBeanFactory beanFactory,
		...
		for (Field field : fields_HttpInvoker) {
			...
			if (beanFactory.containsBeanDefinition(beanID)) {
				log.info(String.format("spring bean[%s] has been defined in application context!", beanID));
				return;
			}
	...
	public static void injectSpringBeanRemote(final ApplicationContext ctx, Object testedObject) {
		...
		for (Field field : fields_HttpInvoker) {
			...
			try {
				...
				ReflectUtil.setFieldValue(testedObject, field, bean);
			} catch (Exception e) {
		...
		while (en.hasMoreElements()) {
			...
			for (String type : types) {
				...
				if (type.equalsIgnoreCase("httpinvoker")) {
					...
					log.info("register httpinvoker bean[/httpinvoker/" + bean + "] successfully.");
					...
					log.info("register httpinvoker bean[/hessian/" + bean + "] successfully.");
				} else {
