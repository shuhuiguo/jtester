...
import fitnesse.wikitext.parser.ParserTestHelper;
...
public class WidgetRootTest {
	...
	public void carriageReturnsShouldNotMatterIfPresentOnPage() throws Exception {
		...
		assertEquals("<i>italics</i>" + ParserTestHelper.newLineRendered + "<b>bold</b>"
	...
	public void nestedTableExpansion() throws Exception {
		...
		boolean found = Pattern.compile("<table.+<table.+</table.+</table", Pattern.DOTALL).matcher(html).find();
	...
	public void nestedNewlineExpansion() throws Exception {
		...
		boolean found = Pattern.compile("<table.+<td.+<ul.+<li.+</li.+</ul.+</td.+</table", Pattern.DOTALL)
				.matcher(html).find();
