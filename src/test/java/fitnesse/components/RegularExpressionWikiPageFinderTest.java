...
public class RegularExpressionWikiPageFinderTest implements SearchObserver {
	...
	public void setUp() throws Exception {
		...
		childPage = crawler.addPage(root, PathParser.parse("PageOne.PageOneChild"), "PageChild is a child of PageOne");
		...
		data.setAttribute(WikiPageProperties.VIRTUAL_WIKI_ATTRIBUTE, "http://localhost:" + FitNesseUtil.port
				+ "/PageOne");
	...
	private Matcher<List<WikiPage>> found(final WikiPage... pages) {
		return new TypeSafeMatcher<List<WikiPage>>() {
			...
			public boolean matchesSafely(List<WikiPage> foundPages) {
				if (foundPages.size() != pages.length)
					return false;

				for (WikiPage expectedPage : pages) {
					if (!foundPages.contains(expectedPage))
						return false;
				}
				return true;
			}
