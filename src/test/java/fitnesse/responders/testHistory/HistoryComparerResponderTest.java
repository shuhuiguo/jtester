...
public class HistoryComparerResponderTest {
	...
	private final String FIRST_FILE_PATH = "./TestDir/files/testResults/TestFolder/firstFakeFile".replace('/',
			File.separatorChar);
	private final String SECOND_FILE_PATH = "./TestDir/files/testResults/TestFolder/secondFakeFile".replace('/',
			File.separatorChar);
	...
	public void shouldReturnErrorPageIfCompareFails() throws Exception {
		...
		assertHasRegexp("These files could not be compared.  They might be suites, or something else might be wrong.",
				response.getContent());
	}
	...
	public void shouldReturnErrorPageIfThereAreTooFewInputFiles() throws Exception {
		...
		SimpleResponse response = (SimpleResponse) responder.makeResponse(context, request);
		assertEquals(400, response.getStatus());
		assertHasRegexp("Compare Failed because the wrong number of Input Files were given. Select two please.",
				response.getContent());
	}
