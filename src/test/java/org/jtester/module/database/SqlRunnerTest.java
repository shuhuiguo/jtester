...
public class SqlRunnerTest extends JTester {
	...
	public void testQuery_ForMap() throws SQLException {
		...
		want.map(map).notNull().hasEntry("id", 1, "firstName", "dddd");
		Timestamp time = (Timestamp) map.get("myDate");
	...
	public void testQuery_ForPoJo() throws SQLException {
		...
		want.object(user).notNull().propertiesMatch(new String[] { "id", "firstName" }, new Object[] { 1, "dddd" });
	...
	public void testExecuteFromFile() throws SQLException, FileNotFoundException {
		fit.executeSQLFile("org/jtester/module/database/data/SqlRunnerTest/testExecuteFromFile.sql");
	}
