...
/**
 * Custom test runner that will Unitils-enable your test. This will make sure
 * that the core unitils test listener methods are invoked in the expected
 * order. See {@link TestListener} for more information on the listener
 * invocation order.
 * <p/>
 * NOTE: if a test fails, the error is logged as debug logging. This is a
 * temporary work-around for a problem with IntelliJ JUnit-4 runner that reports
 * a 'Wrong test finished' error when something went wrong in the before.
 * [IDEA-12498]
 * 
 * @author Tim Ducheyne
 * @author Filip Neven
...
public class UnitilsJUnit4TestClassRunner extends JUnit4ClassRunner {
	...
	/**
	 * Overridden JUnit4 method to be able to create a CustomMethodRoadie that
	 * will invoke the unitils test listener methods at the appropriate moments.
	 */
	protected void invokeTestMethod(Method method, RunNotifier notifier) {
		...
		if (!testMethod.isIgnored()) {
			try {
				getTestListener().setupClass(testObject);
			} catch (Throwable e) {
	...
	/**
	 * Returns the JUnit 4 MethodRoadie object that is used to execute the test
	 * method
	 * 
	 * @param testObject
	 ...
	 * @param jUnitTestMethod
	 *            The JUnit test method
	 * @param notifier
	 *            The run listener, not null
	 * @param description
	 *            A test description
	 * @return An JUnit MethodRoadie
	 */
	protected MethodRoadie createMethodRoadie(Object testObject, Method testMethod, TestMethod jUnitTestMethod,
	...
	/**
	 * Custom method roadie that invokes the unitils test listener methods at
	 * the apropriate moments.
	 */
	protected class TestListenerInvokingMethodRoadie extends MethodRoadie {
		 ...
		 *            A test description
		 */
		public TestListenerInvokingMethodRoadie(Object testObject, Method testMethod, TestMethod jUnitTestMethod,
				RunNotifier notifier, Description description) {
		 ...
		 * {@link TestListener#afterTestTearDown}.
		 */
		@Override
		public void runBeforesThenTestThenAfters(Runnable test) {
			try {
				getTestListener().setupMethod(testObject, testMethod);
			} catch (Throwable t) {
				addFailure(t);
			}
			if (throwable == null) {
			...
			try {
				getTestListener().teardownMethod(testObject, testMethod);
			} catch (Throwable t) {
				addFailure(t);
			}
		}
		...
		protected void runTestMethod() {
			try {
				getTestListener().beforeMethodRunning(testObject, testMethod);
			} catch (Throwable t) {
				addFailure(t);
			}
			if (throwable == null) {
			...
			try {
				getTestListener().afterMethodRunned(testObject, testMethod, throwable);
			} catch (Throwable t) {
				addFailure(t);
			}
		}
		...
		protected void addFailure(Throwable t) {
			// first exception is typically the most meaningful, so ignore
			// second exception
			if (throwable == null) {
				throwable = t;
				super.addFailure(t);
	 ...
	 */
	protected CoreModule getUnitils() {
