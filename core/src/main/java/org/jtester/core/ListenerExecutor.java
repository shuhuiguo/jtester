...
public class ListenerExecutor {
	 ...
	 * @return 事件异常
	 */
	public static Throwable executeSetupClass(Object testedObject) {
		String hits = String.format(TEST_CLAZZ_INFO, "Begin", testedObject.getClass().getName(), Thread.currentThread()
				.getId());
		JTesterLogger.info("\n\n\n" + hits);

		try {
			getTestListener().setupClass(testedObject);
			return null;
		} catch (Throwable e) {
			e.printStackTrace();
			return e;
