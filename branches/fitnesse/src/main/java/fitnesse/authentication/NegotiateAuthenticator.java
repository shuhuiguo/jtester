...
public class NegotiateAuthenticator extends Authenticator {
	...
	protected boolean stripRealm = true; /*
										 * Strip the realm off the authenticated
										 * user's name
										 */
	...
	protected void initServiceCredentials() throws Exception {
		if (serviceName == null)
			...
			serverCreds = manager.createCredential(name, GSSCredential.INDEFINITE_LIFETIME, mechanism,
	 ...
	 * Otherwise, stores the next token to send in the password field and sets
	 * request username to null. XXX It would be better to allow associating
	 * generic authenticator data to each request.
	 */
	protected void negotiateCredentials(Request request) throws Exception {
	...
	private void setCredentials(Request request, byte[] inputToken) throws GSSException, UnsupportedEncodingException {
		/*
		 * XXX Nowhere to attach a partial context to a TCP connection, so we
		 * are limited to single-round auth mechanisms.
		 */
