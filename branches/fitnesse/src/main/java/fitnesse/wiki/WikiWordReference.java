...
public class WikiWordReference {
	...
	private String expandPrefix(String theWord) throws Exception {
		...
		if (theWord.charAt(0) == '^' || theWord.charAt(0) == '>') {
			...
			// todo rcm, this loop is duplicated in
			// PageCrawlerImpl.getSiblingPage
	...
	private String getQualifiedWikiWord(String wikiWordText) throws Exception {
		...
		if (expandedPath == null)
			return wikiWordText;
		WikiPagePath fullPath = currentPage.getParent().getPageCrawler()
				.getFullPathOfChild(currentPage.getParent(), expandedPath);
		return "." + PathParser.render(fullPath); // todo rcm 2/6/05 put that
													// '.' into pathParser.
													// Perhaps
													// WikiPagePath.setAbsolute()
	}
	...
	public void wikiWordRenamePageIfReferenced(Symbol wikiWord, WikiPage pageToRename, String newName) throws Exception {
		...
		if (refersTo(fullPathToReferent, absolutePathToPageBeingRenamed)) {
			...
			String renamedReference = makeRenamedRelativeReference(wikiWord.getContent(),
					PathParser.parse(fullRenamedPathToReferent));
