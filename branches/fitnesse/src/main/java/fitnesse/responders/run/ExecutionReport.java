...
public abstract class ExecutionReport {
	...
	public static ExecutionReport makeReport(String xmlString) throws Exception {
		...
		if (documentNodeName.equals("testResults"))
			...
			throw new RuntimeException(String.format("%s is not a valid document element tag for an Execution Report.",
					documentNodeName));
	}
	...
	private void unpackFinalCounts(Element testResults) throws Exception {
		...
		if (counts != null) {
			finalCounts = new TestSummary(Integer.parseInt(XmlUtil.getTextValue(counts, "right")),
					Integer.parseInt(XmlUtil.getTextValue(counts, "wrong")), Integer.parseInt(XmlUtil.getTextValue(
							counts, "ignores")), Integer.parseInt(XmlUtil.getTextValue(counts, "exceptions")));
		}
	}
