...
public abstract class SlimTestSystem extends TestSystem implements SlimTestContext {
	...
	private String processTablesAndGetHtml(List<SlimTable> tables, SlimTable startWithTable, SlimTable nextTable)
			throws Exception {
		expectations.clear();
	...
	static String translateExceptionMessage(String exceptionMessage) {
		...
		if (tokens[0].equals("COULD_NOT_INVOKE_CONSTRUCTOR"))
			...
			return String.format("The instruction %s is malformed",
					exceptionMessage.substring(exceptionMessage.indexOf(" ") + 1));
