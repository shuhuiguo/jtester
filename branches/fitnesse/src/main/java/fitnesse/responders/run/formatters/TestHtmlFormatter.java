...
public abstract class TestHtmlFormatter extends BaseFormatter {
	...
	public TestHtmlFormatter(FitNesseContext context, final WikiPage page, final HtmlPageFactory pageFactory)
			throws Exception {
		super(context, page);
	...
	protected HtmlPage buildHtml(String pageType) throws Exception {
		...
		html.header.add(String.format("&nbsp;<a style=\"font-size:small;\" href=\"%s?pageHistory\"> [history]</a>",
				fullPathName));
	...
	protected String makeSummaryContent() {
		...
		if (latestTestTime != null) {
			summaryContent = String.format("%s<strong>Assertions:</strong> %s (%.03f seconds)", testPageSummary(),
					assertionCounts, latestTestTime.elapsedSeconds());
		} else {
