...
public class HistoryComparer {
	...
	public boolean allTablesMatch() {
		if (matchedTables == null || matchedTables.size() == 0 || firstTableResults == null
				|| firstTableResults.size() == 0)
			return false;
	...
	private void incrementRemaingMatchesToCompensateForInsertion(ResultAdjustmentStrategy adjustmentStrategy,
			int currentMatch) {
	...
	private boolean firstAndSecondTableAreNotBlank(int tableIndex) {
		return !(firstTableResults.get(tableIndex).equals(blankTable) || secondTableResults.get(tableIndex).equals(
				blankTable));
	}
