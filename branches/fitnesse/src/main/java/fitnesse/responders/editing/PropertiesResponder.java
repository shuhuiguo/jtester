...
public class PropertiesResponder implements SecureResponder {
	...
	private JSONObject makeJson() throws Exception {
		...
		String attributes[] = new String[] { TEST.toString(), PropertySEARCH, PropertyEDIT, PropertyPROPERTIES,
				PropertyVERSIONS, PropertyREFACTOR, PropertyWHERE_USED, PropertyRECENT_CHANGES, SUITE.toString(),
				PropertyPRUNE, PropertySECURE_READ, PropertySECURE_WRITE, PropertySECURE_TEST };
	...
	public HtmlTag makePageTypeRadiosHtml(PageData pageData) throws Exception {
		return makeAttributeRadiosHtml("Page type: ", PAGE_TYPE_ATTRIBUTES, PAGE_TYPE_ATTRIBUTE, pageData);
	}
	...
	private HtmlTag makeSymbolicLinkSection() throws Exception {
		...
		table.addRow(new HtmlElement[] { new HtmlTag("strong", "Name"), new HtmlTag("strong", "Path to Page"),
	...
	private void addSymbolicLinkRows(HtmlTableListingBuilder table) throws Exception {
		...
		for (String linkName : symbolicLinkNames) {
			...
			// ---Unlink---
			HtmlTag actionItems = HtmlUtil
					.makeLink(resource + "?responder=symlink&removal=" + linkName, "Unlink&nbsp;");
			// ---Rename---
	...
	public HtmlTag makeTestActionCheckboxesHtml(PageData pageData) throws Exception {
		return makeAttributeCheckboxesHtml("Actions:", ACTION_ATTRIBUTES, pageData);
	}

	public HtmlElement makeNavigationCheckboxesHtml(PageData pageData) throws Exception {
		return makeAttributeCheckboxesHtml("Navigation:", NAVIGATION_ATTRIBUTES, pageData);
	}

	public HtmlTag makeSecurityCheckboxesHtml(PageData pageData) throws Exception {
		return makeAttributeCheckboxesHtml("Security:", SECURITY_ATTRIBUTES, pageData);
	}
	...
	public HtmlTag makeHelpTextHtml(PageData pageData) throws Exception {
		return makeInputField("Help Text:", PropertyHELP, "HelpText", 90, pageData);
	}
