...
public class TOCWidget extends WikiWidget {
	...
	private boolean recursive, isGraceful, isVarGraceful, isPropertied, isVarPropertied, isFiltered, isVarFiltered,
	...
	private HtmlTag buildListItem(WikiPage wikiPage, int currentDepth) throws Exception {
		...
		boolean willRecurse = isRecursive() && buildListOfChildPages(wikiPage).size() > 0, depthExceeded = isDepthExceeded(currentDepth + 1);
	...
	private HtmlElement getLinkText(WikiPage wikiPage) throws Exception {
		String name = regrace(wikiPage.getName()), props = getProperties(wikiPage), filters = getFilters(wikiPage);
	...
	private String getProperties(WikiPage wikiPage) throws Exception {
		...
		if (isPropertyAppended()) {
			...
			if (props.has("Suite"))
				propText.append(propertyCharacters.charAt(0));
			if (props.has("Test"))
				propText.append(propertyCharacters.charAt(1));
			if (props.has("WikiImport"))
				propText.append(propertyCharacters.charAt(2));
			if (isSymbolic(wikiPage))
				propText.append(propertyCharacters.charAt(3));
			// ...try to keep the following property last in the list!
			if (props.has("Prune"))
				propText.append(propertyCharacters.charAt(4));
		}
	...
	private List<WikiPage> buildListOfChildPages(WikiPage wikiPage) throws Exception {
		...
		if (wikiPage.hasExtension(VirtualCouplingExtension.NAME)) {
			VirtualCouplingExtension extension = (VirtualCouplingExtension) wikiPage
