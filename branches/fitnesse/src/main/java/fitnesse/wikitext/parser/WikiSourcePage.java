...
public class WikiSourcePage implements SourcePage {
			...
			if (includedPage == null) {
				if (page instanceof ProxyPage) {
					...
					try {
						...
						return Maybe
								.nothingBecause("Remote page \" + host + \":\" + port + \"/\" + pageName + \" does not exist.\n");
					}
				} else {
					return Maybe.nothingBecause("Page include failed because the page " + pageName
							+ " does not exist.\n");
				}
	...
	public Collection<SourcePage> getAncestors() {
		...
		try {
			for (WikiPage ancestor = page.getParent(); ancestor != null && ancestor != page; ancestor = ancestor
					.getParent()) {
