...
public class Include extends SymbolType implements Rule, Translation {
	...
	public Maybe<Symbol> parse(Symbol current, Parser parser) {
		...
		if (includedPage.isNothing()) {
			...
			ParsingPage included = option.equals("-setup") || option.equals("-teardown") ? parser.getPage() : parser
					.getPage().copyForNamedPage(includedPage.getValue());
	...
	public String toTarget(Translator translator, Symbol symbol) {
		...
		if (option.equals("-seamless")) {
			...
			String title = "Included page: "
					...
					+ " "
					+ new WikiWordBuilder(translator.getPage(), symbol.childAt(1).getContent(), "(edit)").buildLink(
							"?edit&amp;redirectToReferer=true&amp;redirectAction=", symbol.childAt(1).getContent());
	...
	private String stateForOption(String option, Symbol symbol) {
		return ((option.equals("-setup") || option.equals("-teardown")) && symbol.getVariable("COLLAPSE_SETUP", "true")
				.equals("true")) || option.equals("-c") ? Collapsible.Closed : Collapsible.Open;
	}
}
