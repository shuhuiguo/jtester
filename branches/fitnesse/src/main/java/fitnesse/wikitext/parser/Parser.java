...
public class Parser {
	...
	public static Parser make(ParsingPage currentPage, String input, VariableSource variableSource,
		...
		return new Parser(null, currentPage, new Scanner(new TextMaker(variableSource, currentPage.getNamedPage()),
				input), variableSource, specification);
	}
	...
	public Parser(Parser parent, ParsingPage currentPage, Scanner scanner, VariableSource variableSource,
	...
	public List<Symbol> peek(SymbolType[] types) {
		...
		for (int i = 0; i < lookAhead.size(); i++) {
			if (!lookAhead.get(i).isType(types[i]))
				return emptySymbols;
		}
		return lookAhead;
	}
	...
	public Symbol parseWithParent(String input, Parser parent) {
		return new Parser(parent, currentPage, new Scanner(new TextMaker(variableSource, currentPage.getNamedPage()),
				input), variableSource, new ParseSpecification().provider(specification)).parse();
	}
	...
	public Symbol parse() {
		...
		while (true) {
			...
			if (specification.terminatesOn(currentToken.getType()))
				break;
	...
	private boolean parentOwns(SymbolType current, ParseSpecification specification) {
		if (parent == null)
			return false;
		if (parent.specification.hasPriority(specification) && parent.specification.terminatesOn(current))
			return true;
