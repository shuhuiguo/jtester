...
public class PlainTextTable extends SymbolType implements Rule {
	...
	private static final SymbolType[] terminators = new SymbolType[] { SymbolType.PlainTextCellSeparator,
			SymbolType.Newline, SymbolType.ClosePlainTextTable };
	...
	public Maybe<Symbol> parse(Symbol current, Parser parser) {
		...
		if (!parser.getCurrent().isType(SymbolType.Newline) && !parser.getCurrent().isType(SymbolType.Whitespace)) {
			...
			plainTextTableTypes = new SymbolProvider(new SymbolType[] { SymbolType.Newline,
					SymbolType.ClosePlainTextTable, Evaluator.symbolType, Literal.symbolType, Variable.symbolType });
	...
	private class ColumnSeparator implements Matchable {
		...
		public SymbolMatch makeMatch(ScanString input) {
			...
			return matchLength.isNothing() ? SymbolMatch.noMatch : new SymbolMatch(SymbolType.PlainTextCellSeparator,
					input, matchLength.getValue());
		}
	}
}
