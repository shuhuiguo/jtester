...
public class FitClient {
			...
			while (!finishedReading()) {
				...
				if (size != 0) {
					...
					if (fitOutput.byteCount() < size)
						throw new Exception("I was expecting " + size + " bytes but I only got "
								+ fitOutput.byteCount());
	 ...
	 *         <p/>
	 *         When the number of pages sent and recieved is the same, we may be
	 *         done with the whole job, or we may just be waiting for FitNesse
	 *         to send the next page. There's no way to know until FitNesse
	 *         either calls send, or done.
	 */
