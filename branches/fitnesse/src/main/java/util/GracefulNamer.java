...
public class GracefulNamer {
	...
	// todo OH FRATZ! Graceful names was for java method and instance names, not
	// wiki page names! Get all this wiki page name stuff out of here.
	public static String regrace(String disgracefulName) {
		...
		for (int i = 1; i < disgracefulName.length(); i++) {
			...
			if ((Character.isUpperCase(c)) || (Character.isDigit(c) && !isGrabbingDigits) || (c == separator)) {
				if (!wasSeparator)
