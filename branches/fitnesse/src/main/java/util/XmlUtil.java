...
public class XmlUtil {
	...
	public static Document newDocument(InputStream input) throws Exception {
		try {
			...
			throw new Exception(String.format("SAXParseException at line:%d, col:%d, %s", e.getLineNumber(),
	...
	public static Document newDocument(File input) throws Exception {
		try {
			return getDocumentBuilder().parse(
					new InputSource(new InputStreamReader(new FileInputStream(input), "UTF-8")));
		} catch (SAXParseException e) {
			throw new Exception(String.format("SAXParseException at line:%d, col:%d, %s", e.getLineNumber(),
					e.getColumnNumber(), e.getMessage()));
		}
	...
	public static String getElementText(Element namedElement) throws Exception {
		...
		// throw new Exception("No child of " + namedElement.getNodeName() +
		// " is a Text node");
		return null;
	}
