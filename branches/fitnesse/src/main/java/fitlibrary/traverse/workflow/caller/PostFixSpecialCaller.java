...
public class PostFixSpecialCaller extends AbstractDoCaller {
	...
	public PostFixSpecialCaller(Row row, Evaluator evaluator, boolean sequencing) {
		// Warning: Hack to fix conflict between "set" and "=", by giving "set"
		// precedence.
		String firstCell = row.text(0, evaluator);
		if (row.size() == 4
				&& "=".equals(row.text(2, evaluator))
				&& ("set".equals(firstCell) || "setSymbolNamed".equals(evaluator.getRuntimeContext().extendedCamel(
						firstCell))))
			return;
		if (row.size() >= 2) {
