...
public class ParallelSuiteResult implements SuiteResult {
	...
	public ParallelSuiteResult(String name, boolean showPasses) {
		...
		content = new StringBuffer("<head><title>")
				.append(name)
				.append("</title><link rel='stylesheet' type='text/css' href='fitnesse.css' media='screen'/>"
	...
	private void appendRow(TestResult result, Counts resultCounts) {
		...
		content.append("</td><td>").append(resultCounts.right).append("</td><td>").append(resultCounts.wrong)
				.append("</td><td>").append(resultCounts.exceptions).append("</td><td>")
				.append(result.durationMillis()).append("</td></tr>");
	}

	private void appendNameOrLink(TestResult result) {
		if (showPasses || (result.getCounts().exceptions + result.getCounts().wrong) > 0)
			content.append("<a href='").append(result.getName()).append(".html'>").append(result.getName())
					.append("</a>");
		else
