...
public class Diff_match_patch {
	 ...
	 * @return Linked List of Diff objects.
	 */
	public LinkedList<Diff> diff_main(String text1Initial, String text2Initial, boolean checklines) {
	 ...
	 * @return Linked List of Diff objects.
	 */
	protected LinkedList<Diff> diff_compute(String text1Initial, String text2Initial, boolean checkLinesInitial) {
		String text1 = text1Initial;
		String text2 = text2Initial;
			...
			while (true) {
				if (v_map.get(d).contains(diff_footprint(x - 1, y))) {
					...
					if (last_op == Operation.DELETE) {
						...
						path.addLast(new Diff(Operation.DELETE, text1.substring(text1.length() - x - 1, text1.length()
								- x)));
					}
					last_op = Operation.DELETE;
					break;
					...
					if (last_op == Operation.INSERT) {
						...
						path.addLast(new Diff(Operation.INSERT, text2.substring(text2.length() - y - 1, text2.length()
								- y)));
					}
					last_op = Operation.INSERT;
					break;
					...
					if (last_op == Operation.EQUAL) {
						...
						path.addLast(new Diff(Operation.EQUAL, text1.substring(text1.length() - x - 1, text1.length()
								- x)));
					}
					last_op = Operation.EQUAL;
				}
	...
	public void diff_cleanupSemanticLossless(LinkedList<Diff> diffs) {
		...
		while (nextDiff != null) {
			if (prevDiff.operation == Operation.EQUAL && nextDiff.operation == Operation.EQUAL) {
				...
				while (!"".equals(edit) && !"".equals(equality2) && edit.charAt(0) == equality2.charAt(0)) {
	...
	public LinkedList<Patch> patch_make(String text1, LinkedList<Diff> diffs) {
		...
		for (Diff aDiff : diffs) {
			...
			case DELETE:
				...
				break;
			case EQUAL:
				if (aDiff.text.length() <= 2 * Patch_Margin && !"".equals(patch.diffs) && aDiff != diffs.getLast()) {
	 ...
	 *         boolean values.
	 */
	public Object[] patch_apply(LinkedList<Patch> patchesInitial, String textInitial) {
	...
	public void patch_splitMax(LinkedList<Patch> patches) {
		...
		while (bigpatch != null) {
			...
			while (!"".equals(bigpatch.diffs)) {
				...
				while (!"".equals(bigpatch.diffs) && patch.length1 < patch_size - Patch_Margin) {
				...
				if (!"".equals(postcontext)) {
					...
					if (!"".equals(patch.diffs) && patch.diffs.getLast().operation == Operation.EQUAL) {
