...
public class StoryXmlBuilderTest extends JTester {
	...
	public void testAddScenario() throws Exception {
		Element scenarios = (Element) document.selectSingleNode(StoryQName.xpathScenarios);
	...
	public void testAddTemplateMethod() throws Exception {
		Element scenario = (Element) document.selectSingleNode(StoryQName.xpathTemplates);
		...
		want.string(xml)
				.contains(
						"<template name='我的准备工作' type='given'><![CDATA[具体描述]]><para name='参数'><![CDATA[ddd]]></para><![CDATA[ddd]]></template>",
						StringMode.SameAsSpace, StringMode.SameAsQuato);
	}
	...
	public void testModifyTemplateMethod() {
		Element template = (Element) document.selectSingleNode(StoryQName.xpathTemplatesMethod + "[1]");
		...
		want.string(xml)
				.contains(
						"<templates>"
								+ // <br>
								"<template type='then' name='新的准备方法'><![CDATA[新的描述]]><para name='新的参数'><![CDATA[ddd]]></para><![CDATA[ddd]]></template>"
								+ // <br>
								"<template type='then' name='do template method2'>模板内容</template>" + // <br>
								"</templates>", StringMode.IgnoreSpace, StringMode.SameAsQuato);
	}
	...
	Iterator dataConvertPathID() {
		return new DataIterator() {
			{
				data("/story", "/story");
				data("/story/templates/template", "/story/templates/template[1]");
				data("/story/templates/template[2]", "/story/templates/template[2]");
				data("/story/scenarios/scenario", "/story/scenarios/scenario[1]");
				data("/story/scenarios/scenario/method", "/story/scenarios/scenario[1]/method[1]");
				data("/story/scenarios/scenario[2]/method", "/story/scenarios/scenario[2]/method[1]");
				data("/story/scenarios/scenario[2]/method[1]", "/story/scenarios/scenario[2]/method[1]");
			}
		};
	}
	...
	public void testCpyStoryScenario() throws Exception {
		StoryXmlBuilder.cpyStoryScenario(document, StoryQName.xpathScenarios + "/scenario[1]", new StoryScenario(
				"我拷贝的场景").setDescription("新的描述"));
		String xml = XmlHelper.toXml(document);
