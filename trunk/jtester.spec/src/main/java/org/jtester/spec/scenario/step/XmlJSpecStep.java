...
public class XmlJSpecStep extends JSpecStep {
	...
	public XmlJSpecStep(String scenario, Element methodNode, List<JSpecStep> templates) {
		super(scenario);
		...
		JSpecStep template = JSpecStep.findTemplate(templates, this.method, this.type);
		this.hasTemplate = template != null;
		this.parseStep(methodNode, template);
	}

	@Override
	public void parseStep(Object content, JSpecStep template) {
	...
	private void parseStepType(Element element) {
		...
		if (StringHelper.isEmpty(type)) {
			throw new RuntimeException(
					"illegal step type, the method type must be one of following values: given, when, then. but actual is null.");
		}
		type = type.trim();
	 ...
	 * @return
	 */
	protected String getText(Element element, JSpecStep template) {
		String templateText = "";
		if (template != null) {
