...
public class XmlJSpecScenario extends JSpecScenario {
	...
	public XmlJSpecScenario(Element scenarioNode, List<JSpecStep> templates) {
	 ...
	 * @param templates
	 */
	private void parse(Element scenarioNode, List<JSpecStep> templates) {
	...
	private static List<JSpecScenario> parseJSpecScenarioFrom(Document document) {
		List<JSpecScenario> scenarios = new ArrayList<JSpecScenario>();

		List<JSpecStep> templates = parseJSpecTemplate(document);
	 ...
	 * @return
	 */
	private static List<JSpecStep> parseJSpecTemplate(Document document) {
		List<JSpecStep> templates = new ArrayList<JSpecStep>();
		List<Element> templateNodes = document.selectNodes(StoryQName.xpathTemplatesMethod);
	 ...
	 * @param is
	 * @param encoding
	 *            文本流编码，如果为null，则自动获取，如果自动获取失败，则使用默认编码
	 * @return
	 */
	public static List<JSpecScenario> parseJSpecScenarioFrom(InputStream is, String encoding) {
		try {
			...
			List<JSpecScenario> scenarios = parseJSpecScenarioFrom(document);
			return scenarios;
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public static List<JSpecScenario> parseJSpecScenarioFrom(String xml, String encoding) {
