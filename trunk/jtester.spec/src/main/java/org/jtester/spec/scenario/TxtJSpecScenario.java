...
public class TxtJSpecScenario extends JSpecScenario {
	...
	public TxtJSpecScenario(List<String> lines, List<JSpecStep> templates) {
		this.parse(lines, templates);
	}

	private void parse(List<String> lines, List<JSpecStep> templates) {
		StringBuilder buff = new StringBuilder();
		JSpecStep step = null;
		JSpecStep template = null;
	 ...
	 * @param step
	 */
	private void addStep(StringBuilder buff, JSpecStep step, JSpecStep template) {
		if (step == null) {
	...
	static List<JSpecScenario> parseJSpecScenarioFrom(LinesReader reader) {
		...
		try {
			...
			do {
				...
				if (line == null || LineType.isScenarioLine(line)) {
					JSpecScenario scenario = new TxtJSpecScenario(lines, storyFeature.getTemplates());
	 ...
	 * @param lines
	 */
	private static void addTemplate(List<JSpecStep> templates, List<String> lines) {
	...
	public static List<JSpecScenario> parseJSpecScenarioFrom(String story) {
		String lines[] = story.split("\n");
		LinesReader reader = new StringLinesReader(lines);
		List<JSpecScenario> scenarios = parseJSpecScenarioFrom(reader);
		return scenarios;
	}
