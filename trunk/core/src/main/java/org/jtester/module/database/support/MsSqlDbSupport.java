 ...
 * <p/>
 * Special thanks to Niki Driessen who donated the initial version of the Derby
 * support code.
 * 
 * @author Tim Ducheyne
...
public class MsSqlDbSupport extends DbSupport {
	...
	public Set<String> getTableNames() {
		...
		Set set = handler
				.getItemsAsStringSet("select t.name from sys.tables t, sys.schemas s where t.schema_id = s.schema_id and s.name = '"
						+ getSchemaName() + "'");
		return set;
	}
	...
	public Set<String> getColumnNames(String tableName) {
		SQLHandler handler = getSQLHandler();
		Set set = handler
				.getItemsAsStringSet("select c.name from sys.columns c, sys.tables t, sys.schemas s where c.object_id = t.object_id and t.name = '"
	...
	public Set<String> getViewNames() {
		SQLHandler handler = getSQLHandler();
		Set set = handler
				.getItemsAsStringSet("select v.name from sys.views v, sys.schemas s where v.schema_id = s.schema_id and s.name = '"
						+ getSchemaName() + "'");
		return set;
	}
	...
	public Set<String> getSynonymNames() {
		SQLHandler handler = getSQLHandler();
		Set set = handler
				.getItemsAsStringSet("select o.name from sys.synonyms o, sys.schemas s where o.schema_id = s.schema_id and s.name = '"
						+ getSchemaName() + "'");
		return set;
	}
	...
	public Set<String> getTriggerNames() {
		SQLHandler handler = getSQLHandler();
		Set set = handler
				.getItemsAsStringSet("select t.name from sys.triggers t, sys.all_objects o, sys.schemas s where t.parent_id = o.object_id and o.schema_id = s.schema_id and s.name = '"
						+ getSchemaName() + "'");
		return set;
	}
	...
	public Set<String> getTypeNames() {
		SQLHandler handler = getSQLHandler();
		Set set = handler
				.getItemsAsStringSet("select t.name from sys.types t, sys.schemas s where t.schema_id = s.schema_id and s.name = '"
						+ getSchemaName() + "'");
		return set;
	}
	...
	public Set<String> getIdentityColumnNames(String tableName) {
		SQLHandler handler = getSQLHandler();
		Set set = handler
				.getItemsAsStringSet("select i.name from sys.identity_columns i, sys.tables t, sys.schemas s where i.object_id = t.object_id and t.name = '"
						+ tableName + "' and t.schema_id = s.schema_id and s.name = '" + getSchemaName() + "'");
		return set;
	}
	...
	protected void disableReferentialConstraints(String tableName) {
		...
		Set<String> constraintNames = sqlHandler
				.getItemsAsStringSet("select f.name from sys.foreign_keys f, sys.tables t, sys.schemas s where f.parent_object_id = t.object_id and t.name = '"
						+ tableName + "' and t.schema_id = s.schema_id and s.name = '" + getSchemaName() + "'");
		for (String constraintName : constraintNames) {
			sqlHandler.executeUpdate("alter table " + qualified(tableName) + " drop constraint "
	...
	protected void disableValueConstraints(String tableName) {
		...
		Set<String> keyConstraintNames = sqlHandler
				.getItemsAsStringSet("select k.name from sys.key_constraints k, sys.tables t, sys.schemas s where k.type = 'UQ' and k.parent_object_id = t.object_id and t.name = '"
						+ tableName + "' and t.schema_id = s.schema_id and s.name = '" + getSchemaName() + "'");
		for (String keyConstraintName : keyConstraintNames) {
			sqlHandler.executeUpdate("alter table " + qualified(tableName) + " drop constraint "
					+ quoted(keyConstraintName));
		}

		// disable all check constraints
		Set<String> checkConstraintNames = sqlHandler
				.getItemsAsStringSet("select c.name from sys.check_constraints c, sys.tables t, sys.schemas s where c.parent_object_id = t.object_id and t.name = '"
						+ tableName + "' and t.schema_id = s.schema_id and s.name = '" + getSchemaName() + "'");
		for (String checkConstraintName : checkConstraintNames) {
			sqlHandler.executeUpdate("alter table " + qualified(tableName) + " drop constraint "
					+ quoted(checkConstraintName));
		}
	...
	public void incrementIdentityColumnToValue(String tableName, String identityColumnName, long identityValue) {
		...
		getSQLHandler()
				.executeUpdate("DBCC CHECKIDENT ('" + qualified(tableName) + "', reseed, " + identityValue + ")");
	}
	 ...
	 * <p/>
	 * For primary keys, row-guid, identity and computed columns not-null
	 * constrains cannot be disabled in MS-Sql.
	 * 
	 * @param tableName
	 *            The table, not null
	 */
	protected void disableNotNullConstraints(String tableName) {
		...
		// retrieve the name of the primary key, since we cannot remove the
		// not-null constraint on this column
		Set<String> primaryKeyColumnNames = sqlHandler
				.getItemsAsStringSet("select c.name from sys.key_constraints k, sys.index_columns i, sys.columns c, sys.tables t, sys.schemas s "
		...
		try {
			...
			// get all not-null columns but not row-guid, identity and computed
			// columns (these cannot be altered in MS-Sql)
			resultSet = statement
					.executeQuery("select c.name column_name, upper(y.name) data_type, c.max_length, c.precision from sys.types y, sys.columns c, sys.tables t, sys.schemas s "
							+ "where c.is_nullable = 0 and c.is_rowguidcol = 0 and c.is_identity = 0 and c.is_computed = 0 "
							+ "  and y.user_type_id = c.user_type_id and c.object_id = t.object_id and t.name = '"
							+ tableName + "' and t.schema_id = s.schema_id and s.name = '" + getSchemaName() + "'");
