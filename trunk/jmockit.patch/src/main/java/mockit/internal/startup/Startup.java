...
import org.jtester.junit.jmockit.JUnit4TestRunnerDecorator;
import org.jtester.junit.jmockit.MockAllDefaultPossibilitiesBuilder;
...
/**
 * This is the "agent class" that initializes the JMockit "Java agent". It is
 * not intended for use in client code. It must be public, however, so the JVM
 * can call the {@code premain} method, which as the name implies is called
...
public final class Startup {
	...
	/**
	 * This method must only be called by the JVM, to provide the
	 * instrumentation object. In order for this to occur, the JVM must be
	 * started with "-javaagent:jmockit.jar" as a command line parameter
	 * (assuming the jar file is in the current directory).
	 * <p/>
	 * It is also possible to load other <em>instrumentation tools</em> at this
	 * time, according to any agent arguments provided as
	 * "-javaagent:jmockit.jar=agentArgs" in the JVM command line. There are two
	 * types of instrumentation tools:
	 * <ol>
	 * <li>A {@link ClassFileTransformer class file transformer}, which will be
	 * instantiated and added to the JVM instrumentation service. Such a class
	 * must be public and have a public constructor accepting two parameters:
	 * the first of type {@code Map&lt;String, byte[]>}, which will receive a
	 * map for storing the transformed classes; and the second of type
	 * {@code String}, which will receive any tool arguments.</li>
	 * <li>An <em>external mock</em>, which can be any class with a public
	 * no-args constructor. Such a class will be used to redefine one or more
	 * real classes. The real classes can be specified in one of two ways: by
	 * providing a regular expression matching class names as the tool
	 * arguments, or by annotating the external mock class with
	 * {@link mockit.MockClass}.</li>
	 * </ol>
	 * 
	 * @param agentArgs
	 *            zero or more <em>instrumentation tool specifications</em>
	 *            (separated by semicolons if more than one); each tool
	 *            specification must be expressed as
	 *            "&lt;tool class name>[=tool arguments]", with fully qualified
	 *            class names for classes available in the classpath; tool
	...
	private static void preventEventualClassLoadingConflicts() {
		// Pre-load certain JMockit classes to avoid NoClassDefFoundError's when
		// mocking certain JRE classes,
		// such as ArrayList.
		try {
	...
	private static void loadInternalStartupMocksForJUnitIntegration() {
		if (setUpInternalStartupMock(TestSuiteDecorator.class)) {
			...
			// modify by darui.wu 2011-12-25
			setUpInternalStartupMock(MockAllDefaultPossibilitiesBuilder.class);
			setUpInternalStartupMock(JUnit4TestRunnerDecorator.class);
			// end modify by darui.wu
	...
	public static boolean initializeIfNeeded() {
		if (instrumentation == null) {
			try {
				...
				e.printStackTrace(); // makes sure the exception gets printed at
										// least once
				throw e;
			}
		}
