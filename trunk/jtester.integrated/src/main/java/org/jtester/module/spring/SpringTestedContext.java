...
public class SpringTestedContext {
	...
	public static void setSpringContext(JTesterSpringContext springContext) {
		if (springContext == null) {
			MessageHelper.info("no spring application context for test:" + ObjectTestedContext.currTestedClazzName());
			return;
		}
		if (ObjectTestedContext.currTestedClazz() == null) {
			throw new RuntimeException("the tested object can't be null.");
		} else {
			springBeanFactories.put(ObjectTestedContext.currTestedClazz(), springContext);
		}
	}
	...
	public static JTesterBeanFactory getSpringBeanFactory() {
		if (ObjectTestedContext.currTestedClazz() == null) {
			...
			JTesterSpringContext springContext = springBeanFactories.get(ObjectTestedContext.currTestedClazz());
	...
	public static void removeSpringContext() {
		if (ObjectTestedContext.currTestedClazz() == null) {
			...
			SpringBeanCleaner.cleanSpringBeans(ObjectTestedContext.currTestedObject());
			Object springContext = springBeanFactories.remove(ObjectTestedContext.currTestedClazz());
			if (springContext != null) {
