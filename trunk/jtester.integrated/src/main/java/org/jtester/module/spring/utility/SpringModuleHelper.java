...
public class SpringModuleHelper {
	...
	public static Object getBeanByName(String beanname) {
		...
		if (factory == null) {
			throw new RuntimeException("can't find SpringApplicationContext for tested class:"
					+ ObjectTestedContext.currTestedClazzName());
		} else {
	...
	public static JTesterSpringContext initSpringContext(Class testClazz, ApplicationContextFactory contextFactory) {
		...
		SpringContext annotation = AnnotationHelper.getClassLevelAnnotation(SpringContext.class,
				testClazz);
		Class declaredAnnotationClazz = AnnotationHelper.getClassWithAnnotation(SpringContext.class,
				testClazz);
		if (annotation == null) {
			return null;
		}
	...
	private static JTesterSpringContext newSpringContext(Class testClazz, ApplicationContextFactory contextFactory,
		...
		boolean shared = annotation.share();
	...
	public static void closeSpringContext(Object springContext) {
		...
		if (context.isShared() == false) {
			context.destroy();
			MessageHelper.warn("close spring context for class:" + ObjectTestedContext.currTestedClazzName());
		}
	}
