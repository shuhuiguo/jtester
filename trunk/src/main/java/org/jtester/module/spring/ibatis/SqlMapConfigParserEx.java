...
public class SqlMapConfigParserEx extends SqlMapConfigParser {
	...
	protected static final Logger log = Logger.getLogger(SqlMapConfigParserEx.class);
	private XmlParserState state = new XmlParserState();

	@Override
	protected void addSqlMapNodelets() {
		this.state = (XmlParserState) FieldAccessor.getFieldValue(
				com.ibatis.sqlmap.engine.builder.xml.SqlMapConfigParser.class, this, "state");
	...
	private static class JTesterIbatisNodelet implements Nodelet {
		...
		public void process(Node node) {
			...
			try {
				...
				boolean throwException = ConfigUtil.boolProperty(ConfigUtil.IBATIS_SQLMAP_THROW_EXCEPTION);
				if (throwException) {
