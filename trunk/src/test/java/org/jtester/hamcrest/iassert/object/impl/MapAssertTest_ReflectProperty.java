...
public class MapAssertTest_ReflectProperty extends JTester {
	...
	public void testMapReflet_matcher() {
		Map<String, String> maps = mockMap();
		want.map(maps).propertyEq("one", the.string().isEqualTo("my first value"));
	}
	...
	public void testMapRefletList_PropertyCollection() {
		want.collection(list).propertyCollectionMatch(
				"one",
	...
	public void testMapRefletList_PropertyCollection_order() {
		try {
			want.collection(list).propertyCollectionMatch("one",
					the.collection().reflectionEq(new String[] { "ddd", "my first value" }));
			want.fail();
		} catch (Throwable e) {
	...
	public void propertyCollectionRefEq() {
		String[][] expecteds = new String[][] { { "my first value", "my third value" }, 
				{ "ddd", "my third value" } };
		want.collection(list).propertyCollectionRefEq(new String[] { "one", "three" }, expecteds,
				ReflectionComparatorMode.LENIENT_ORDER);
	}
