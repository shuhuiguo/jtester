...
public class ReflectionAssertTest extends JTester {
	...
	public void propertyMatch_formap_NoKey() {
		...
		try {
			...
			want.object(e).matchToString(the.string().contains("can't find property"));
		}
	}
	...
	public void propertyMatchOne_forlist() {
		List<Employee> list = createEmployee();
		want.collection(list).sizeEq(4).matchAnyProperty("name", the.string().contains("name 1"));
	}
	...
	public void testAllPropertyMatch() {
		...
		want.array(users).matchAllProperty("first", the.string().contains("abc"));
	}

	@Test
	public void testPropertiesMatch() {
		List<User> users = Arrays.asList(new User("dfa123sdf", "abc"), new User("firs123tname", "abc"));
		want.collection(users).matchAllProperty("first", the.string().contains("123"))
