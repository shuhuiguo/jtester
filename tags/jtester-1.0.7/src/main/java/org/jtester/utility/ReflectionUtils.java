...
import org.jtester.module.utils.TypeUtils;
...
public class ReflectionUtils {
	...
	public static <T> T createInstanceOfType(Class<T> type, boolean bypassAccessibility, Class[] argumentTypes,
		...
		try {
			...
			throw new JTesterException("Error while trying to create object of class " + type.getName(), e.getCause());
	...
	public static void setFieldValue(Object object, Field field, Object value) {
		try {
			...
			throw new JTesterException("Unable to assign the value to field: " + field.getName()
	...
	public static void setFieldAndSetterValue(Object object, Set<Field> fields, Set<Method> setterMethods, Object value) {
		...
		for (Method method : setterMethods) {
			if (!isSetter(method)) {
				throw new JTesterException("Method " + method.getName()
			...
			try {
				...
				throw new JTesterException("Unable to invoke method: " + object.getClass().getSimpleName() + "."
	...
	/**
	 * Invoke the given method with the given parameters on the given target
	 * object. Doesn't throw any checked exception
	 * 
	 * @param target
	 *            The object containing the method, not null
	 * @param method
	 ...
	 * @return The result of the invocation, null if void
	 * @throws JTesterException
	 *             if the method could not be invoked, or the called method
	...
	public static Method getSetter(Class<?> clazz, String propertyName, boolean isStatic) {
		...
		Set<Method> allMethods = getAllMethods(clazz);
		for (Method method : allMethods) {
			if (isSetter(method) && setterName.equals(method.getName()) && isStatic == isStatic(method.getModifiers())
	...
	/**
	 * From the given class, returns the getter for the given propertyname. If
	...
	public static String getPropertyName(Method setterMethod) {
		...
		if (methodName.length() < 4 || !methodName.startsWith("set")) {
			throw new JTesterException("Unable to get field name for setter method " + setterMethod);
		}
