...
public class JTesterReflector {
	...
	public <T> T invoke(Class<?> clazz, Object target, String method, Object... paras) {
		...
		Object _target = ReflectUtil.getProxiedObject(target);
		return (T) ReflectUtil.invoke(clazz, _target, method, paras);
	}
	...
	public <T> T invokeStatic(Class<?> target, String method, Object... paras) {
		return (T) ReflectUtil.invoke(target, null, method, paras);
	}
	...
	public <T> T getSpringAdvisedTarget(Object source) {
		Object target = ReflectUtil.getProxiedObject(source);
		return (T) target;
	}

	/**
	 * 返回target对象field字段的代理实例，用于运行时转移代理操作到字段对象上
	 * 
	 * @param <T>
	 * @param target
	 * @param field
	 * @return
	 */
	public <T> T getFieldProxy(Object target, String fieldname) {
		if (target == null) {
