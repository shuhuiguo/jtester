...
public abstract class UnitilsJUnit3 extends TestCase {
	...
	/**
	 * Keeps track of the test class for which tests are currently being
	 * executed.
	 */
	private static Class<?> currentTestClass;

	/**
	 * Creates a test without a name. Be sure to call {@link TestCase#setName}
	 * afterwards.
	 */
	...
	/**
	 * Creates a test with the given name. The name should be the name of the
	 * test method.
	 * 
	 * @param name
	 ...
	 * {@link TestListener#beforeTestSetUp} and
	...
	protected void runTest() throws Throwable {
		...
		// if an exception occured during beforeTestMethod, the test or
		// afterTestMethod, throw it
		if (firstThrowable != null) {
			throw firstThrowable;
		}
	}
	 ...
	 * {@link ModuleHelper#getInstance()}.
	 * <p/>
	 * You can override this method to let it create and set your own singleton
	 * instance. For example, you can let it create an instance of your own
	 * Unitils subclass and set it by using {@link ModuleHelper#setInstance}.
	 * 
	...
	protected Method getCurrentTestMethod() {
		...
		if (StringUtils.isEmpty(testName)) {
			throw new JTesterException(
					"Unable to find current test method. No test name provided (null) for test. Test class: "
							+ getClass());
		}
		...
		try {
			...
			throw new JTesterException("Unable to find current test method. Test name: " + getName()
					+ " , test class: " + getClass(), e);
		}
	}
