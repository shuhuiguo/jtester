...
public class SpringBeanRegister {
	private static Logger log = Logger.getLogger(SpringBeanRegister.class);
	...
	protected final void registerSpringBean() throws FindBeanImplClassException {
		...
		for (Field field : fields_SpringBeanByName) {
			...
			if (claz != SpringBeanByName.class && ClazzUtil.isInterfaceOrAbstract(claz) == false) {
		...
		for (Field field : fields_SpringBeanByType) {
			...
			String beanName = field.getName();
			if (claz != SpringBeanByType.class && ClazzUtil.isInterfaceOrAbstract(claz) == false) {
	...
	protected Class<?> registerBeanDefinition(final String beanName, final Class<?> ownerClazz,
			final Class<?> fieldType, boolean lazyInit, boolean override) throws FindBeanImplClassException {
		...
		if (beanClazz == null || ClazzUtil.isInterfaceOrAbstract(beanClazz)) {
			return null;
		}
		try {
	...
	protected void injectPropertyBean(final Class<?> ownerClazz) {
		...
		for (Method method : all) {
			...
			try {
				...
				if (ignore) {
					log.warn("ignore NotFound:" + e.getMessage());
				} else {
					throw new RuntimeException(e);
				}
			}
