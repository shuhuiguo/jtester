...
//import org.jtester.monitor.asm.ClassEnhancer;
...
public class TracerModuleHelper {
	...
	public static boolean traceJDBC() {
		boolean traceJDBC = ConfigUtil.boolProperty(TRACE_JDBC_KEY);
		return traceJDBC;
	}

	public static boolean traceSpringBean() {
		boolean traceBean = ConfigUtil.boolProperty(TRACE_SPRING_KEY);
		return traceBean;
	}
	...
	public static File getAboutMethodFile(Object testObject, Method testMethod, String surfix) {
		...
		if (htmlFile.getParentFile().exists() == false) {
	...
	public static void writeTracerInfo(File htmlFile, String jpgFile, String beanTracerInfo, String jdbcTracerInfo) {
		try {
			...
			writer.append(String.format("<META HTTP-EQUIV='Content-Type' CONTENT='text/html; charset=%s'>",
					ResourceUtil.fileEncoding()));
			writer.append("<style>");
			InputStream is = ClassLoader.getSystemResourceAsStream("org/jtester/testng/UserTestReporter.css");

			if (is != null) {
				String style = ResourceUtil.convertStreamToString(is);
				writer.append(style);
			}
