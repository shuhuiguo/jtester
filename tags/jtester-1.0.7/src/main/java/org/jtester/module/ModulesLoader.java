...
import static org.jtester.module.utils.PropertyUtils.getBoolean;
 ...
 * will be loaded. These names can then be used to construct properties that
 * define the classnames and optionally the dependencies of these modules. E.g.
 * 
 * <pre>
 * <code>
 ...
 * <p/>
 * If a circular dependency is found in the runAfter configuration, a runtime
...
public class ModulesLoader {
	...
	/**
	 * Last part of the core specific property that specifies the names of the
	 * modules that should be run before this core.
	 */
	...
	public List<Module> loadModules(Properties configuration) {
		...
		for (String moduleName : moduleNames) {
			...
			List<String> runAfterModuleNames = getStringList(PROPKEY_MODULE_PREFIX + moduleName
					+ PROPKEY_MODULE_SUFFIX_RUN_AFTER, configuration);
			runAfters.put(moduleName, runAfterModuleNames);
		}

		// Count each time a core is (indirectly) used in runAfter and order by
		// count
		...
		for (String moduleName : moduleNames) {
			...
			// store in map with count as key and a list corresponding modules
			// as values
	...
	/**
	 * Creates the modules with the given class names and calls initializes them
	...
	protected List<Module> createAndInitializeModules(List<String> moduleNames, Properties configuration) {
		...
		for (String moduleName : moduleNames) {
			...
			String className = getString(PROPKEY_MODULE_PREFIX + moduleName + PROPKEY_MODULE_SUFFIX_CLASS_NAME,
					configuration);
			if (!classFileExistsInClasspath(className)) {
				logger.debug("Skipping module " + moduleName
						+ ". Module class not found in classpath. Module class name: " + className);
				continue;
			}
			try {
				...
				if (!(module instanceof Module)) {
					throw new JTesterException("Unable to load core. Module class is not of type UnitilsModule: "
							+ className);
				}
				// initialize module
	 ...
	 * count.
	 * <p/>
	 * This way all modules can be ordered in such a way that all core
	 * dependencies (runAfterz) are met. If no such order can be found (circular
	 * dependency) a runtime exception is thrown
	 * 
	 * @param moduleName
	 *            the core to count, not null
	 * @param allRunAfters
	 *            all dependencies as (moduleName, run-after moduleNames)
	 *            entries, not null
	 * @param traversedModuleNames
	 *            all moduleNames that were already counted as (moduleName,
	 *            moduleName) entries, not null
	 * @return the count
	 * @throws RuntimeException
	 *             if an infinite loop (circular dependency) is found
	 */
	private int countRunAfters(String moduleName, Map<String, List<String>> allRunAfters,
			Map<String, String> traversedModuleNames) {
		// Check for infinite loops
		if (traversedModuleNames.containsKey(moduleName)) {
			throw new JTesterException("Unable to load modules. Circular dependency found for modules: "
					+ traversedModuleNames.keySet());
		}
	...
	protected void removeDisabledModules(Set<String> moduleNames, Properties configuration) {
		...
		while (moduleNameIterator.hasNext()) {
			...
			boolean enabled = getBoolean(PROPKEY_MODULE_PREFIX + moduleName + PROPKEY_MODULE_SUFFIX_ENABLED, true,
					configuration);
			if (!enabled) {
