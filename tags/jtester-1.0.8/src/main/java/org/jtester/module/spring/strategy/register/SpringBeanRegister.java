...
public class SpringBeanRegister {
	final static Logger log4j = Logger.getLogger(SpringBeanRegister.class);
	...
	protected final void registerSpringBean() throws FindBeanImplClassException {
		...
		for (Field field : fields_SpringBeanByName) {
			...
			if (claz != SpringBeanByName.class && ClazzHelper.isInterfaceOrAbstract(claz) == false) {
				RootBeanDefinition beanDefinition = SpringBeanRegister.getRootBeanDefinition(beanName, claz, false);
				...
				RootBeanDefinition beanDefinition = SpringBeanRegister.getRootBeanDefinition(beanName, impl, false);
	 ...
	 * @return
	 */
	public static RootBeanDefinition getRootBeanDefinition(String beanName, Class implClazz, boolean isLazy) {
	 ...
	 * @return
	 */
	public static AnnotatedGenericBeanDefinition getAnnotatedGenericBeanDefinition(String beanName, Class implClazz,
			boolean isLazy) {
		AnnotatedGenericBeanDefinition beanDefinition = new AnnotatedGenericBeanDefinition(implClazz);
		// beanDefinition.setBeanClassName(implClazz.getName());
		beanDefinition.setScope("singleton");
		beanDefinition.setAutowireCandidate(true);
		beanDefinition.setLazyInit(isLazy);
